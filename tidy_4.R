...
ave_scores = read_csv("tidy_2_ave_scores.csv")
...
perf_bump_plot = ggplot(perf_bump) +
                 ...
                 scale_x_continuous("Average Points Gained Event 1 -> Event 2", labels=scales::percent) +
...
ggsave("tidy_4_perf_bump_plot.png",perf_bump_plot,width=5.5,height=4,units='in',dpi=300)

evt_scores_plot = ggplot(perf_bump) +
                  stat_ecdf(aes(x=first_score),color='red') +
                  stat_ecdf(aes(x=second_score),color='blue') +
                  theme_bw() +
                  scale_x_continuous("Event Score") +
                  scale_y_continuous("Cumulative Distribution", labels=scales::percent) +
                  scale_color_discrete("Week")

ggsave("tidy_4_evt_scores_plot.png",evt_scores_plot,width=5.5,height=4,units='in',dpi=300)
...
ggsave("tidy_4_pts_year_plot.png",pts_year_plot,width=5.5,height=4,units='in',dpi=300)
